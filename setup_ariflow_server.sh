# 讀取環境檔案
source .env
sudo rm -rf "$AIRFLOW_VOLUMES" "$STORAGE"

# 生成 AIRFLOW 需要的資料夾以及 AIRFLOW_UID
mkdir -p "$AIRFLOW_DAGS" "$AIRFLOW_LOGS" "$AIRFLOW_PLUGINS" "$AIRFLOW_VOLUMES" "$STORAGE"
sudo chown -R $USER:$USER "$AIRFLOW_DAGS"
sudo chown -R $USER:$USER "$AIRFLOW_PLUGINS"
